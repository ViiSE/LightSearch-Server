...
@Scope("prototype")
public class LoggerServerImpl implements LoggerServer {

    private final LoggerFile loggerFile;
    private final LoggerWindow loggerWindow;
    private final CurrentDateTime currentDateTime;

    public LoggerServerImpl(
            @Qualifier("loggerFile") LoggerFile loggerFile,
    ...
    private void log(Class<?> clazz, LogMessageTypeEnum type, String message) {
        if(message != null) {
            if (!message.isEmpty()) {
                loggerFile.write(type.stringValue(), currentDateTime,
