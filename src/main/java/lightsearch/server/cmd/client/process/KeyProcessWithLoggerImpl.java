...
@Component("keyProcessWithLogger")
public class KeyProcessWithLoggerImpl implements ClientProcess<ClientCommandResult> {

    private final LoggerServer logger;
    private final ClientProcess<ClientCommandResult> clientProcess;

    public KeyProcessWithLoggerImpl(
            LoggerServer logger,
            @Qualifier("keyRsaProcess") ClientProcess<ClientCommandResult> clientProcess) {
        this.logger = logger;
    ...
    public ClientCommandResult apply(ClientCommand command) {
        ...
        logger.info(KeyRSAProcess.class, result.logMessage());
        return result;
    }
}
