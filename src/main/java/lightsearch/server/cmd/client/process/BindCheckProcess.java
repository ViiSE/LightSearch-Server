...
@Component("bindCheckProcess")
public class BindCheckProcess implements ClientProcess<ClientCommandResult> {
    ...
    public BindCheckProcess(
            @Qualifier("commandCheckerClientBindCheck") Checker<ClientCommand> checker,
    ...
    public ClientCommandResult apply(ClientCommand command) {
        try {
            ...
            DatabaseCommandMessage dbCmdMessage =
                    dbCmdMsgProducer.getDatabaseCommandMessageBindCheckWindowsJSONInstance(command);
