...
@Component("bindCheckProcessWithLogger")
public class BindCheckProcessWithLoggerImpl implements ClientProcess<ClientCommandResult> {

    private final LoggerServer logger;
    private final ClientProcess<ClientCommandResult> clientProcess;

    public BindCheckProcessWithLoggerImpl(
            LoggerServer logger,
            @Qualifier("bindCheckProcess") ClientProcess<ClientCommandResult> clientProcess) {
        this.logger = logger;
    ...
    public ClientCommandResult apply(ClientCommand command) {
        ...
        if (!result.isDone())
            logger.error(BindCheckProcess.class, result.logMessage());
