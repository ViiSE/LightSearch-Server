...
@Component("openSoftCheckProcess")
public class OpenSoftCheckProcess implements ClientProcess<ClientCommandResult> {
    ...
    public OpenSoftCheckProcess(
            @Qualifier("commandCheckerClientOpenSoftCheck") Checker<ClientCommand> checker,
    ...
    public ClientCommandResult apply(ClientCommand command) {
        try {
            ...
            DatabaseCommandMessage dbCmdMessage =
                    dbCmdMsgProducer.getDatabaseCommandMessageOpenSoftCheckWindowsJSONInstance(command);

            ClientCommandResult cmdRes = cmdExec.exec(dbCmdMessage);

            return resultProducer.getClientCommandResultOpenSoftCheckInstance(cmdRes);
