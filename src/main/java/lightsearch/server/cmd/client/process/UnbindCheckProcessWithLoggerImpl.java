...
@Component("unbindCheckProcessWithLogger")
public class UnbindCheckProcessWithLoggerImpl implements ClientProcess<ClientCommandResult> {

    private final LoggerServer logger;
    private final ClientProcess<ClientCommandResult> clientProcess;

    public UnbindCheckProcessWithLoggerImpl(
            LoggerServer logger,
            @Qualifier("unbindCheckProcess") ClientProcess<ClientCommandResult> clientProcess) {
        this.logger = logger;
    ...
    public ClientCommandResult apply(ClientCommand command) {
        ...
        if (!result.isDone())
            logger.error(UnbindCheckProcess.class, result.logMessage());
