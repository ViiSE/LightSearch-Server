...
@Component("cancelSoftCheckProcessWithLogger")
public class CancelSoftCheckProcessWithLoggerImpl implements ClientProcess<ClientCommandResult> {

    private final LoggerServer logger;
    private final ClientProcess<ClientCommandResult> clientProcess;

    public CancelSoftCheckProcessWithLoggerImpl(
            LoggerServer logger,
            @Qualifier("cancelSoftCheckProcess") ClientProcess<ClientCommandResult> clientProcess) {
        this.logger = logger;
    ...
    public ClientCommandResult apply(ClientCommand command) {
        ...
        if (!result.isDone())
            logger.error(CancelSoftCheckProcess.class, result.logMessage());
