...
@Component("tkListProcessWithLogger")
public class TKListProcessWithLoggerImpl implements ClientProcess<ClientCommandResult> {

    private final LoggerServer logger;
    private final ClientProcess<ClientCommandResult> clientProcess;

    public TKListProcessWithLoggerImpl(
            LoggerServer logger,
            @Qualifier("tkListProcess") ClientProcess<ClientCommandResult> clientProcess) {
        this.logger = logger;
    ...
    public ClientCommandResult apply(ClientCommand command) {
        ...
        if (!result.isDone())
            logger.error(TKListProcess.class, result.logMessage());
