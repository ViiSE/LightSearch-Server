...
@Component("skladListProcessWithLogger")
public class SkladListProcessWithLoggerImpl implements ClientProcess<ClientCommandResult> {

    private final LoggerServer logger;
    private final ClientProcess<ClientCommandResult> clientProcess;

    public SkladListProcessWithLoggerImpl(
            LoggerServer logger,
            @Qualifier("skladListProcess") ClientProcess<ClientCommandResult> clientProcess) {
        this.logger = logger;
    ...
    public ClientCommandResult apply(ClientCommand command) {
        ...
        if (!result.isDone())
            logger.error(SkladListProcess.class, result.logMessage());
