...
@Component("unbindProcessWithLogger")
public class UnbindProcessWithLoggerImpl implements ClientProcess<ClientCommandResult> {

    private final LoggerServer logger;
    private final ClientProcess<ClientCommandResult> clientProcess;

    public UnbindProcessWithLoggerImpl(
            LoggerServer logger,
            @Qualifier("unbindProcess") ClientProcess<ClientCommandResult> clientProcess) {
        this.logger = logger;
    ...
    public ClientCommandResult apply(ClientCommand command) {
        ...
        if (!result.isDone())
            logger.error(UnbindProcess.class, result.logMessage());
