 ...
 * @author ViiSE
 */
@Component("changeClientTimeoutProcess")
public class ChangeClientTimeoutProcess implements AdminProcess<AdminCommandResult> {
    ...
    public ChangeClientTimeoutProcess(
            AdminCommandResultProducer admCmdResProducer,
            @Qualifier("lightSearchClientTimeoutPropertyCreator") PropertyCreator<String, AdminCommand> propertyCreator,
            PropertiesChanger<Void, Property<String>> propertiesChanger,
            @Qualifier("commandCheckerAdminClientTimeout") Checker<AdminCommand> checker) {
    ...
    synchronized public AdminCommandResult apply(AdminCommand command) {
        try {
            ...
            return admCmdResProducer.getAdminCommandResultSimpleInstance(
                    true,
                    "Client timeout value has been changed. Need restart server for changed apply.");
        } catch (PropertiesException | CheckerException ex) {
            return admCmdResProducer
