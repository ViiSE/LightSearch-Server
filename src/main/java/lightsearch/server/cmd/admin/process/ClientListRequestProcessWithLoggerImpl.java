...
@Component("clientListRequestProcessWithLogger")
public class ClientListRequestProcessWithLoggerImpl implements AdminProcess<AdminCommandResult> {

    private final LoggerServer logger;
    private final AdminProcess<AdminCommandResult> adminProcess;

    public ClientListRequestProcessWithLoggerImpl(
            LoggerServer logger,
            @Qualifier("clientListRequestProcess") AdminProcess<AdminCommandResult> adminProcess) {
        this.logger = logger;
    ...
    public AdminCommandResult apply(AdminCommand command) {
        ...
        logger.info(AdminCommandsController.class, "Admin request client list");
        return result;
    }
}
