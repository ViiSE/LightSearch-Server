...
@Service("clientProducerDefault")
public class ClientProducerDefaultImpl implements ClientProducer {

    private final ApplicationContext ctx;

    public ClientProducerDefaultImpl(ApplicationContext ctx) {
        this.ctx = ctx;
    }

    @Override
    public Client getClientSimpleInstance(String IMEI, String username) {
        return (Client) ctx.getBean("clientSimple", IMEI, username);
    }

    @Override
    public Client getClientWithJWTTokenInstance(Client client, JWTTokenHeader tokenHeader) {
        return (Client) ctx.getBean("clientWithJwtToken", client, tokenHeader);
    }
}
