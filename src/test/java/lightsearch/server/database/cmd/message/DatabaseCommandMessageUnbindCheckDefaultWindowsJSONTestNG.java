...
public class DatabaseCommandMessageUnbindCheckDefaultWindowsJSONTestNG {

    DatabaseCommandMessage dbCmdMessage;

    @BeforeClass
    @Parameters({"barcode"})
    public void setUpClass(String barcode) {
        String command = ClientCommands.UNBIND_CHECK;

        ClientCommand clientCommand = new ClientCommandWithBarcodeImpl(
                new ClientCommandSimpleImpl(command),
                barcode);

        dbCmdMessage = new DatabaseCommandMessageUnbindCheckDefaultWindowsJSONImpl(clientCommand);

        testBegin(DatabaseCommandMessageUnbindCheckDefaultWindowsJSONImpl.class);
    }
    ...
    public void teardownClass() {
        testEnd(DatabaseCommandMessageUnbindCheckDefaultWindowsJSONImpl.class);
    }
}
