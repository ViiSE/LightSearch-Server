...
public class LSResponseRepositoryFirebirdIntegrationTestNG extends AbstractTestNGSpringContextTests {

    @Autowired
    private ObjectMapper mapper;

    @Autowired
    @Qualifier("lsResponseRepositoryLongFirebird")
    private LSResponseRepository responseRepository;

    @Test
    public void exec() {
        try {
            ResponseResult responseResult = responseRepository.read(1);
            System.out.println(mapper
                    .writerWithDefaultPrettyPrinter()
                    .writeValueAsString(responseResult.formForSend()));
