...
public class ClientCommandWithUserIdentifierTestNG {

    @Test
    @Parameters({"userIdent"})
    public void formForSend(String userIdent) throws JsonProcessingException {
        testBegin(ClientCommandWithUserIdentifierImpl.class, "formForSend()");
        ...
        String ui = cmdDTO.getUserIdentifier();

        assertNotNull(cmd, "Command name is null!");
        assertEquals(cmd, ClientCommands.LOGIN);
        assertNotNull(ui, "user identifier is null!");
        assertEquals(ui, userIdent);

        System.out.println(new ObjectMapper().writeValueAsString(cmdDTO));

        testEnd(ClientCommandWithUserIdentifierImpl.class, "formForSend()");
    }
}
